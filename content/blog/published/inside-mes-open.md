...
This is the same call I made on TrialEdge (LightGBM over neural net at 208 labeled outcomes). I would make the same call for any production system where the labeled set is in the hundreds or low thousands and the operator needs to defend the system's outputs to a non-technical stakeholder.
...
I am leaning toward the third option. The reasoning generalizes: any production system that includes a research-grade component should run that component in a parallel research path, not in the production pipeline. It stays there until the research path produces enough signal to justify production inclusion.
...
These five are the architectural patterns I would apply to a forecasting system, a fraud-detection system, a recommendation system, or a clinical-prediction system. They apply to any domain where the labeled sample is in the hundreds-to-low-thousands range, the operator needs to defend the output, and a human discretionary baseline exists and meaningfully outperforms the model. They are the closest thing I have to a portable methodology.
...
MES Open is the project in my portfolio that most directly shows how I make architectural decisions in a high-noise low-sample domain. In that domain the inputs are limited, the human baseline outperforms the model, and the cost of being wrong is real.
